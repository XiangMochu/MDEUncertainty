...
class SimpleDecoder(nn.Module):
    def __init__(self, in_chs, feat, enc_mode, depth_range):
        super().__init__()
        self.enc_mode = enc_mode
        self.ups = nn.ModuleList()
        self.in_chs = reversed(in_chs)
        for i in range(len(self.in_chs)-1):
            in_feat = self.in_chs[0]+self.in_chs[1] if i==0 else self.in_chs[i+1] + feat 
            out_feat = feat
            self.ups.append(UpSample(in_feat, out_feat))

    def forward(self, x):
        d = x[-1]
        for i in range(len(self.in_chs)-1):
            d = self.ups[i](d, x[-i-2])
        return d

### from BTS
class BTSDefaultParam:
    max_depth = 10
    dataset = 'nyu'

class BTSDecoder(nn.Module):
    def __init__(self, feat_out_channels, num_features, enc_mode, depth_range, params=BTSDefaultParam):
        super().__init__()
        self.enc_mode = enc_mode
        self.params = params
        self.params.max_depth = depth_range[-1]

        assert len(feat_out_channels) == 5

        conv5_up_ratio = 1 if self.enc_mode == 'swin' else 2
        self.upconv5    = upconv(feat_out_channels[4], num_features, conv5_up_ratio)
        ...
        #                                       nn.ELU())
        self.conv1      = torch.nn.Sequential(nn.Conv2d(num_features // 16 + 4, num_features, 3, 1, 1, bias=False),
                                              nn.ELU())

        # self.get_depth  = torch.nn.Sequential(nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False),
        #                                       nn.Sigmoid())

    def forward(self, features, focal=518.8579):
        ...
        iconv1 = F.upsample(iconv1, scale_factor=0.5, mode='bilinear')
