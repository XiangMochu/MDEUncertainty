...
def main(opts):
    ...
    generator = UNCERTAINTY_MODELS[opts.aleatoric_uncertainty](opts.encoder, opts.decoder, opts.reg_mode, 128, 128, (1e-3, opts.max_depth), False)
...
def train(opts, train_loader, model, optimizer, writers, loss_weight):
    ...
    for i, inputs in enumerate(train_loader):
        ...
        pred_prob = F.interpolate(outputs['prob'][j], size=tgt_depth.shape[-2:], align_corners=True, mode='bilinear')
        pred_depth = F.interpolate(outputs['depth'][j], size=tgt_depth.shape[-2:], align_corners=True, mode='bilinear')
        
        entropy = outputs['entropy'][j]
        entropy = F.interpolate(entropy, size=tgt_depth.shape[-2:], align_corners=True, mode='bilinear')
        
        uncertainty = outputs['uncert'][j]
        ...
        tgt_depth = torch.clamp(tgt_depth, opts.min_depth, opts.max_depth)
...
def val(opts, val_loader, model, epoch, writers):
    ...
    for i, inputs in enumerate(val_loader):
        ...
        outputs = model(tgt_img)['depth'].mean(0)
...
def log(opts, writers, mode, img, gt, outputs, errors):
    ...
    if mode == 'train':
        pred_depth = outputs['depth'].mean(0)
        l1_error = outputs['l1_error']
        uncertainty = outputs['uncert'].mean(0)
