...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, transform=None, is_for_online_eval=False):
        ...
        # this Dataset is only for KITTI
        # assert self.args.dataset == 'kitti'
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            # image_path = reformat_kitti_filename(image_path)
            ...
            if os.path.exists(image_path):
                ...
                image = Image.fromarray(np.zeros((375, 1242, 3), dtype=np.uint8))
                depth_gt = Image.fromarray(np.zeros((10, 10)))
...
class KittiDefaultArg(object):
    ...
    def overwrite(self, opts):
        for k in self.__dict__.keys():
            if k in opts.__dict__.keys():
                self.__dict__[k] = opts.__dict__[k]

def reformat_kitti_filename(path):
    scene = path.split('/')[-4]
    side = path.split('/')[-3].split('_')[-1]
    idx = path.split('/')[-1].split('.')[0]
    reformated = '/'.join(path.split('/')[:5]) + f'/{scene}_{side}/{idx}.jpg'
    return reformated


def get_kittiDataset(opts):
    args = KittiDefaultArg(opts)
    train_set = DataLoadPreprocess(args, 'train', transform=preprocessing_transforms('train'))
    test_set = DataLoadPreprocess(args, 'online_eval', transform=preprocessing_transforms('online_eval'))
